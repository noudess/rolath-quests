...
sub EVENT_SAY {
  ...
  if(($text=~/take the gems/i) && (defined($qglobals{DeliveredToGearo}) && $qglobals{DeliveredToGearo} == 1)) {
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 4766 => 1)) { #Assorted Tailoring Supplies
    ...
    quest::faction(1508, 100); #Traders of the Haven
    quest::faction(1510, 5); #House of Fordel
    quest::faction(1511, 15); #House of Midst
    quest::faction(1512, 15); #House of Stout
