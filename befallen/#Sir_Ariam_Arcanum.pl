...
sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("My thanks for destroying these minions of Befallen's masters. This accursed place rends my soul trying to twist me to its purposes. My time here is short.");
  }
  if($text=~/assist/i) {
    quest::say("Our time is short, hero, for the minions of the Burning Dead will soon sense something is amiss here. Will you perform a task so we might destroy the evil that dwells within Befallen?");
  }
  if($text=~/task/i) {
  ...
  if($text=~/who are you/i || $text=~/what are you/i) {
