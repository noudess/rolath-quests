...
sub EVENT_SAY 
	...
	if ($faction > 7)
		{
		$npc->ClearSpecialAbilities();
		quest::attack($name);
		}
	if (quest::getlevel() < 45)
		{
		quest::say("Yer nothing more than a scrub when it comes to dealing in such matters. You'll need to gain more experience.");
		}
	elsif ($text=~/hail/i) 
		{
