sub EVENT_CLICKDOOR {
  ...
  if($doorid == 61) { #SE tower
    if($status > 79) { #GM status
      $client->MovePC(209, 85, 145, 635, 256);
    }
  ...
  if($doorid == 63) { #SW tower
    if($status > 79) { #GM status
      $client->MovePC(209, -830, -865, 1375, 256);
    }
    elsif($client->KeyRingCheck(9425) || plugin::check_hasitem($client, 9425)) { #Ring of Torden
      if(!$client->KeyRingCheck(9425)) {
        $client->KeyRingAdd(9425);
      ...
      if($raid) {
        ...
        if($raid_group >= 0) {
          for ($count = 0; $count < $raid->RaidCount(); $count++) {
            ...
            if($raid->GetGroup($pc->GetName()) == $raid_group) {
              if($pc->CalculateDistance($x, $y, $z) <= 50) {
                $pc->MovePC(209, -830, -865, 1375, 256);
              }
            }
          }
        }
  ...
  if($doorid == 65) { #NW tower
    if($status > 79) { #GM status
      $client->MovePC(209, -350, -2200, 1955, 510);
    }
    elsif($client->KeyRingCheck(9425) || plugin::check_hasitem($client, 9425)) { #Ring of Torden
      if(!$client->KeyRingCheck(9425)) {
        $client->KeyRingAdd(9425);
      ...
      if($raid) {
        ...
        if($raid_group >= 0) {
          for ($count = 0; $count < $raid->RaidCount(); $count++) {
            ...
            if($raid->GetGroup($pc->GetName()) == $raid_group) {
              if($pc->CalculateDistance($x, $y, $z) <= 50) {
                $pc->MovePC(209, -350, -2200, 1955, 510);
              }
            }
          }
        }
  ...
  if($doorid == 67) { #NE tower
    if($status > 79) { #GM status
      $client->MovePC(209, 150, -1220, 1120, 256);
    }
    elsif($client->KeyRingCheck(9425) || plugin::check_hasitem($client, 9425)) { #Ring of Torden
      if(!$client->KeyRingCheck(9425)) {
        $client->KeyRingAdd(9425);
      ...
      if($raid) {
        ...
        if($raid_group >= 0) {
          for ($count = 0; $count < $raid->RaidCount(); $count++) {
            ...
            if($raid->GetGroup($pc->GetName()) == $raid_group) {
              if($pc->CalculateDistance($x, $y, $z) <= 50) {
                $pc->MovePC(209, 150, -1220, 1120, 256);
              }
            }
          }
        }
