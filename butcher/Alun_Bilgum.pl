...
sub EVENT_SAY
	...
	if ($text =~ /hail/i)
		{
		quest::say("Hello, $name. Would you be interested in assisting me in a special [task]?");
		}
	if ($text =~ /task/i)
		{
		if (($ulevel > 15) && ($ulevel < 25))
			...
			if (quest::istaskactive(14))
				...
				quest::say("Go quickly, you need to speak with Averna Eisley to let them know that you're the Red Beetle. Your contact will know what it means.");
				}
			}
		else
			{
