...
sub EVENT_TIMER {
  ...
  if ($timer eq "move") {
    ...
    if ($mob1) {
      ...
      $mobnpc1->MoveTo(-369, -2628, 180, 0, 1);
    }
    if ($mob2) {
      my $mobnpc2 = $mob2->CastToNPC();
      $mobnpc2->MoveTo(-383, -2615, 180, 0 , 1);
    }
    if ($mob3) {
      my $mobnpc3 = $mob3->CastToNPC();
      $mobnpc3->MoveTo(-369, -2652, 180, 0 , 1);
    }
    if ($mob4) {
      my $mobnpc4 = $mob4->CastToNPC();
      $mobnpc4->MoveTo(-400, -2640, 183, 0 , 1);
    }
    if ($mob5) {
      my $mobnpc5 = $mob5->CastToNPC();
      $mobnpc5->MoveTo(-389, -2649, 183, 0 , 1);
    }
  }
