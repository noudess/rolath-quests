...
sub EVENT_ITEM 
	...
	if (quest::istaskactivityactive(37, 7))
		{
		# Turning in container sets the can't do this again flag.
		if (plugin::check_handin(\%itemcount, 61092 => 1))
			{
			quest::updatetaskactivity(37,7);
			}
		}

	if (quest::istaskactivityactive(37, 14))
		{
		if (plugin::check_handin(\%itemcount, 61143 => 1))
			...
			if(!plugin::has_newbie_stuff($client))
				...
				if ($class eq "Cleric")
					{
					quest::enabletitle(6); 	# Cleric
					quest::shout2("All Hail Divine Protector $name!"); 
					}
				quest::updatetaskactivity(37,14);
				}
			else
				{
				quest::summonitem(61143);
				}
			} 
		}
