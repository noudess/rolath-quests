...
sub EVENT_SAY {
  if ($text=~/hail/i) 
	{
	quest::say("Good day, I have discovered something truly wonderful! If I only had the materials required so I can copy my notes and send them to my teacher.  I cannot go fetch the items myself, I am much too busy working on new inks from the local [flora]");
	}
  if ($text=~/discover/i) 
	{
  ...
  if ($text=~/flora/i) 
	{
	quest::say("I have discovered that twilight bloom petals mixed with a few other ingredients, make a wonderful twilight ink!  I just don't have the time to collect them in bulk.  Say, why don't you go fetch me some, and in return, I'll share some old research components.  I think I have an abundance of pixie wings in one of my packs.  Don't bother me unless you have collected at least 4 blooms!");
	}
}
...
# check for ink of the dark, mechanical pen and white paper
  ...
  elsif(plugin::check_handin(\%itemcount, 63125 => 4))#Twilight Bloom
	{
	quest::say("Ahhh, yes, just what I need.  Here are your pixie wings, now go get me some more!");
	quest::summonitem(13142,3);#Pixie Wing
	}
  else 
	{
