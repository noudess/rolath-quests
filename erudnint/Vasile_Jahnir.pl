...
sub EVENT_ITEM 
	...
	if(plugin::check_handin(\%itemcount, 13078 => 2, 7305 => 2)) # The Power of the Gatecallers
		...
		# xp verified on live
		quest::faction(254,5); #faction with Gate Callers increased
	...
	elsif(plugin::check_handin(\%itemcount, 13895=> 1))  # Rungupp
		{
		...
		quest::faction(254,10); #faction with Gate Callers increased
		quest::faction(266,1); #faction with High Council of Erudin increased
		quest::faction(267,1); #faction with High Guards of Erudin increased
		quest::faction(265,-1); #faction with Heretics decreased
		quest::exp(250);
