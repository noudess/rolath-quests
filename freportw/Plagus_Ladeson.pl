...
sub EVENT_ITEM 
	...
	if ($faction < 5)
		...
		if(plugin::check_handin(\%itemcount, 18814 => 1)) 
			{ # A Sealed Letter ID-18814 filename "NERtoShintl"
	...
	if ($faction == 5)
		{
		if(plugin::check_handin(\%itemcount, 18814 => 1)) 
			{ # A Sealed Letter ID-18814 filename "NERtoShintl"  Item will be lost if not amiable or better
			quest::say("The Steel Warriors have no cause to dislike you, but you have yet to prove your worth to this guild.");
			}
		}
	if ($faction > 5)
