...
sub EVENT_ITEM 
	...
	if($ashenorderfaction > 250 && plugin::check_handin(\%itemcount, 10400=> 1, 1903 => 1, 2299 => 1, 10131 => 1)) 
		{
		quest::ding();
	...
	# Blackened Wand ID-13237 - Blackened Sapphire ID-13238 - Orange Sash of Order ID-10132
	elsif($ashenorderfaction > 499 && plugin::check_handin(\%itemcount, 13237=> 1, 13238 => 1, 10132 => 1)) 
		{
		quest::ding();
		...
		quest::exp("400");
		quest::ding();
		#  Faction hits verified.
		quest::faction(361,75);
		quest::faction(281,11);
		quest::faction(309,3);
		}
	else 
		{
