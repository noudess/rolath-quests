...
sub DoRandomPatchwork
	...
	if ($a <= 20)
		...
		#Patchwork Tunic, Patchwork Cloak, Patchwork Sleeves, Patchwork Pants, Patchwork Boots
		}
	}
...
sub EVENT_ITEM
	{
	if ($faction < 5 && plugin::check_handin(\%itemcount, 12109 => 1))
		{    #Pouch of Pixie Dust
		...
		# Verified
		quest::faction(316, 15);    #Scouts of Tunare
		quest::exp(800)
			; #10% of level 3 experience.  Newbie quest, being a bit generous compared to the guidelines.
		quest::ding();
		}
	elsif (plugin::check_handin(\%itemcount, 16390 => 1))
