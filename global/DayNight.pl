...
sub EVENT_CYCLE
{
	if($zonesn eq 'lakerathe' || $zonesn eq 'freportn')
	{
	...
	if($zonesn eq 'commons' || $zonesn eq 'everfrost' || $zonesn eq 'kithicor' || $zonesn eq 'lfaydark' || $zonesn eq 'northkarana' || $zonesn eq 'qey2hh1' || $zonesn eq 'rathemtn' || $zonesn eq 'riwwi' || $zonesn eq 'southkarana' || $zonesn eq 'eastwastes' || $zonesn eq 'sro')
	{
		if ($zonetime < 600 || $zonetime > 1999) 
		{
