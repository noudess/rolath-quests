...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18700 => 1)) { #Tome of Order and Discord
  ...
  elsif(($class eq "Wizard" || $class eq "Druid") && plugin::check_handin(\%itemcount, 77765 => 3, 77766 => 1)) { #Discordant Crystal Shards x 3, Rolled Discordant Parchment
    ...
    if($class eq "Wizard") {
      quest::summonitem(77659); #Spell: Bloodfields Gate
      quest::summonitem(77660); #Spell: Translocate: Bloodfields
      quest::summonitem(77661); #Spell: Bloodfields Portal
    }
    elsif($class eq "Druid") {
      quest::summonitem(77662); #Spell: Circle of Bloodfields
      quest::summonitem(77663); #Spell: Ring of Bloodfields
    }
    quest::ding();
  }
  else {
