sub EVENT_SAY 
	...
	if($text=~/member of the white rose/i)
		{
		if ($faction < 6)
			{
			...
			{
			quest::say("You are not a respectable rogue $name.  I suggest you make your way back to the lower level.");
			}
		}
	}
...
sub EVENT_ITEM 
	{
	if($faction < 6 && $itemcount{5243} == 4) # Mammoth Calf Hide X 4
		{
		...
		quest::exp("2500");
		quest::ding();
		$reward = quest::ChooseRandom(5238,5239,5242,5236,5233,5231,5240,5241,5234,5232,5235,5237);
		quest::summonitem($reward);
		quest::faction(305,"5"); # Rogues of the White Rose Verified on ZAM
		}
	else
		{
