...
sub EVENT_ITEM 
	...
	if (quest::istaskactivityactive($taskid, 14))
		...
		if (plugin::check_handin(\%itemcount, $robeid => 1))
			...
			if(!plugin::has_newbie_stuff($client))
				...
				quest::enabletitle(31);
				quest::shout2("All Hail $name of the White Rose!"); 
	...
	if (plugin::check_handin(\%itemcount, 20181 => 1, 27424 => 1, 27409 => 1, 27427 => 1)) 
		{
		quest::say("I must say, $name, I am impressed. You have passed your training and should now have a full set of armor to begin your adventures. Take this as a symbol of your dedication and hard work. You have earned it. Your training with me has ended. Farewell.");
