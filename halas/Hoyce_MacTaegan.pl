...
#Min faction required is amiable - verified on live
...
sub EVENT_SAY 
	...
	if($text=~/hail/i) 
		...
		{
		quest::say("Hmmm... I don't know if I believe that.");
		}
	elsif($text=~/keven/i) 
		{
...
sub EVENT_ITEM 
	{
	if($faction <5 && plugin::check_handin(\%itemcount, 13768 => 1)) 
		{ #White Wolf Skin - faction verified on live
	...
	elsif($faction <5 && plugin::check_handin(\%itemcount, 16486 => 3, 2125 => 1)) 
		{ #Silk Thread x 3, Large Tattered Skullcap - faction verified on live
	...
	elsif($faction <5 && plugin::check_handin(\%itemcount, 13897 => 4)) 
		{ #Goblin Ice necklacess x 4 - faction verified on live
		...
		quest::faction(327, 5);  # Shamen of Justice
		quest::faction(328, 1);  # Merchants of Halas
		quest::faction(223, -1);  # Circle Of Unseen Hands
		quest::faction(336, -1);  # Coalition of Tradefolk Underground
		quest::faction(244, -1);  # Ebon Mask
	...
	elsif($faction <5 && plugin::check_handin(\%itemcount, 13915 => 1, 7005 => 1, 8011 => 1)) 
		{ #Gnoll Fang, Giant Snake Fang, Hunting Bow - Faction verified on live
	...
	elsif($faction <5 && plugin::check_handin(\%itemcount, 12223 => 1)) 
		{ #Wrath Orc Wristbands - Faction verified on live
		...
		quest::faction(327, 50);  # Shamen of Justice
		quest::faction(328, 7);  # Merchants of Halas
		quest::faction(223, -7);  # Circle Of Unseen Hands
