sub EVENT_SAY {
  ...
  if($text=~/lion delight/i) {
    quest::say("Lion Delight is a trade secret and brings me much praise.  I'll allow only one per customer.  I'll also honor all as will [pick up the lion meat shipment] with a fine portion."); #Text verified on live
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 13961 => 1)) { #Lion Meat Shipment
    ...
    quest::givecash(0,0,3,0); #Gold x 3
