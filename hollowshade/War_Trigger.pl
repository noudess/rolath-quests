...
my $RandomWar = plugin::RandomRange(1800,7200);
...
sub EVENT_TIMER {
	...
	elsif ($timer eq "ResetZoneTimer")
		{
		quest::stoptimer("ResetZoneTimer");
		RESET_GLOBALS();
		quest::repopzone();
		}
	elsif ($timer eq "TakeOver")
		{
		quest::stoptimer("TakeOver");
...
sub EVENT_SIGNAL 
	...
	if (($qglobals{HollowshadeNorth} == $qglobals{HollowshadeEast}) &&
	   ($qglobals{HollowshadeNorth} == $qglobals{HollowshadeSouth}))
		{
		# Zone in conquered state, only listen to signals >100 .
		if ($signal >= 100) 
			{ 
			#Boss Dead
			quest::setglobal("MoorSavior",$signal-100,7,"F");

			#despawn his aides..
		  	quest::spawn_condition($zonesn, $qglobals{HollowshadeOwner}, 0);

			#Notify the Vah Shir!!
			quest::signalwith(166078, 7, 1);
			quest::signalwith(166079, 7, 1);
			quest::signalwith(166080, 7, 1);
			quest::signalwith(166081, 7, 1);
			quest::signalwith(166082, 7, 1);
			quest::signalwith(166083, 7, 1);
			quest::signalwith(166084, 7, 1);
			$event=0;
			
			quest::settimer("ResetZoneTimer", 120);
			}
		}
...
sub RESET_ZONE {
  ...
  quest::setglobal("HollowshadeOwner", 0 , 7, "F"); # Who won.
  quest::setglobal("MoorSavior",0,7,"F");
  $attacker = "";
  $defender = "";
  $takeover_in_progress=0;
...
sub TAKE_OVER {
	...
	quest::setglobal("HollowshadeOwner", $champion+10, 7, "F"); # Who won.
...
sub START_WAR 
	...
	quest::ze(15, "Calling for a WAR: A: $aa D: $dd");
	...
	if ($dd eq "")
		...
		quest::ze(15, "Random Defender Selected for WAR: D: $defender");
		}
