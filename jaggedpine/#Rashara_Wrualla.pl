sub EVENT_SAY {
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 2) {
  if ($text=~/wait/i) {
    ...
    quest::setglobal("muadalet",3,5,"F");
  }
  }
 ...
 if (defined $qglobals{muadalet} && $qglobals{muadalet} == 10) {
  if ($text=~/most dear/i) {
   quest::say("There is a beast called a girplan, and while it does draw blood, it digs its fangs much deeper. I was traveling with my warder and encountered a girplan that was different somehow. It attacked me and my faithful warder ran to my aid. I did come upon some [luck], though.");
   }
   }
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 10) {
  if ($text=~/luck/i) {
  quest::say("I was able to get some distance when it landed a fatal blow to my companion. But, not only did it draw its blood afterward, it drew its essence, and now I am unable to summon my companion to my aid. I do not understand how it happened and I would have thought it [not possible].");
  }
  }
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 10) {
...
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 2) {
  if (plugin::check_handin(\%itemcount, 57008 =>1 )) {
   ...
   if (plugin::check_handin(\%itemcount, 52907 =>1, 52908 =>1 )) {
     ...
     quest::setglobal("muadalet",11,5,"F");
