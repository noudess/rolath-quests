sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 4)) {
      quest::say("Welcome back, $name. I must congratulate you on your recent completion of all three trials. I must admit that I was unsure of your ability to do as well as you did. Now that you have finished, you have the choice of taking on some [other tasks]. If you'd like, we have a chance now to talk more. You're also more than welcome to attempt the trials again. If so, just let me know you're [ready to test] again and we'll proceed down that path. What'll it be, $name?"); #missing text for the keyword more
    }
    elsif (defined($qglobals{ikky}) && ($qglobals{ikky} == 5)) {
      quest::say("Thanks for bringing those relics to me. I have had a chance to examine them and they lead me to believe that something is going on in the [Temple of the Damned]. Could you help me discover their plans?"); #need live text
    }
    elsif (defined($qglobals{ikky}) && ($qglobals{ikky} == 7)) {
      quest::say("We have come upon another problem. The summoners that you encountered in the Temple of the Damned are working at another location closer to the main temple to summon more vile creatures. Do you think that you can [stop the ritual]?"); #need live text
    }
    else {
  ...
  if (($text=~/other tasks/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    quest::say("If you continue on past the temples you will come to an area called [Martyr's Passage]. Within this area you need to find four different relics of those who have died there. Bring them to me.");
  }
  if (($text=~/martyr's passage/i) && defined($qglobals{ikky}) && ($qglobals{ikky} == 4)) {
    $client->Message(4, "Congratulations!- You've been commissioned to investigate Martyr's Passage.");
  }
  if (($text=~/temple of the damned/i) && defined($qglobals{ikky}) && ($qglobals{ikky} == 5)) {
...
sub EVENT_ITEM {
  if (defined($qglobals{ikky}) && ($qglobals{ikky} == 4)) {
    if (plugin::check_handin(\%itemcount, 60141 => 1, 60142 => 1, 60143 => 1, 60144 => 1)) {
      quest::emote("takes the relics."); #Text made up
      quest::say("Hmm, we are still missing some valuable information. I need you to retrieve the missing piece."); #Text made up
