sub EVENT_SAY 
	...
	if ($faction > 6)
	...
	elsif($text=~/the way/i and $client->GetGlobal("MnkPre") == 2)
		{
		# Monk Epic 1.5/2.0 (Prequest)
	...
	elsif($client->GetGlobal("monk_epic") ==1 && $text=~/vision/i)
		{
		# Monk Epic 1.5
	...
	elsif($client->GetGlobal("monk_epic") ==2 && $text=~/time/i)
		{
		# Monk Epic 1.5
	...
	elsif($client->GetGlobal("monk_epic") ==2 && $text=~/power of the Immortals/i)
		{
		# Monk Epic 1.5  
	...
	elsif($client->GetGlobal("monk_epic") ==3 && $text=~/Vortix/i) 
		{
		...
		quest::spawn2(203433,0,0,$x,$y,$z,$h); # NPC: Vortix
...
sub EVENT_ITEM 
	...
	elsif(plugin::check_handin(\%itemcount, 48122 => 1))
		{
		# Monk Epic 1.5
