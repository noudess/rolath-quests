sub EVENT_SAY
	...
	if ($text =~ /hail/i)
		...
		{
		quest::say("So you have heard of it? Tell me what you know. I would be most amused to hear what a feeble creature such as yourself might know of the staff of Ankexfen.");
		}
	elsif ($text =~ /no/i)
		{
...
sub EVENT_ITEM
	...
	if (plugin::takeItems(6323 => 1, 6324 => 1, 6335 => 1, 6336 => 1))
		...
		if ($faction <= 5)
			{
			quest::emote(
				"cackles with glee as she slowly assembles the runed rods, and with a muttered incantation and a flash of light, disappears, leaving only a note that slowly drifts to the ground."
				);
