...
sub EVENT_ITEM
	...
	if(plugin::check_handin(\%itemcount, 16173 => 1, 19565 => 1, 19564 => 1)) #Deathfist Orc Scalp, Orc Clavicle, Orc Fibula
		...
		if (($class eq "Warrior") && ($race eq "Dark Elf"))
			{
			quest::say("Excellent.  Here is a magical shard, $name.  Use it to slay more!");#text made up
			quest::summonitem(1266); #Rolath shard of Battle
			#factions verified on Live
			quest::faction(270, 15); #Indigo Brotherhood^M
			quest::faction(326,-2); #Emerald Warriors^M
			quest::faction(311,-1); #Steel Warriors^M
			quest::faction(1522,-30); #Primordial Malice^M
			quest::exp(500);
			quest::ding();
			...
			{
			quest::say("You are not a member of the Indigo Brotherhood, begone fool!."); #text made up
