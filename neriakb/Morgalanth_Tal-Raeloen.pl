...
sub EVENT_ITEM 
	...
	if (plugin::check_handin(\%itemcount, 22642 => 1, 22643 => 1, 22644 => 1)) 
		...
		#Verified no faction hits on Live
        quest::exp(500);
        quest::ding();
		}
	...
	if (quest::istaskactivityactive($taskid, 14))
		...
		if (plugin::check_handin(\%itemcount, $robeid => 1))
			...
			if (!plugin::has_newbie_stuff($client))
				{
				quest::incstat(0,0); 	# STR
				quest::incstat(1,0); 	# STA
				quest::incstat(2,0);	# AGI
				quest::incstat(3,0);	# DEX
				quest::incstat(4,6);	# INT
				quest::incstat(5,0);	# WIS
				quest::incstat(6,3);	# CHA
				quest::enabletitle(48);
				quest::shout2("All Hail $name of the Spurned!"); 
