...
sub EVENT_SAY 
	{
	my $poifac = $client->GetModCharacterFactionLevel(340);
...
sub EVENT_ITEM 
	...
	if($poifac > 149 && plugin::check_handin(\%itemcount, 14550 => 4)) 
		{ #Woven Grass Amulets
		quest::emote("You have done well, young initiate. I grant you this medallion invested with the divine hatred of Innoruuk. Wear it proudly as a representation of your importance to these sacred spires. Now, if you will excuse me, I must proceed with the burning of these pathetic halfling symbols of faith.");
	...
	if(plugin::check_handin(\%itemcount, 18756 => 1)) 
		{ #Tattered Note
	...
	if(plugin::check_handin(\%itemcount, 18148 => 1)) 
	...
	# Below faction check is because this is part 2 of Innoruuk Recommendation
	# Seems like you should have been able to do that, to do this, and that
	# one requires > -250
	if($poifac > -250 && plugin::check_handin(\%itemcount, 13113 => 1)) 
