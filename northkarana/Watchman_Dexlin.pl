sub EVENT_SAY
	...
	if ($faction >= 5)
		{
		quest::say("I do not trust you.  I have yet to hear of the great deeds you have done in the name of the Gem Choppers of Ak'Anon.");
		}
	else
		{
		if ($text =~ /Hail/i)
		...
		if ($text =~ /brave fighter/i)
			...
			quest::setglobal("skelup", "0", "3", "F")
				;    # Resets $skelup to 0 for check on next line
			quest::signalwith(13120, 2)
				; # Signals Tiny Skeletons - "asks them" if any of them are spawned, if so they set $skelup = 1 (prevents triggering multiple spawns)
			quest::settimer(1, 10)
				;    # Spawn skeletons after a short delay (if none are up)
			}
		}
	}
...
sub EVENT_ITEM
	...
	if (plugin::check_handin(\%itemcount, 12376) => 1)
		...
		quest::faction(1604, -1);    # Dark Reflection
		quest::exp(10000);
		quest::ding;
		quest::summonitem(12378)
			;  # Give Undead Brownie Bones for turnin to Narron Jenork (Ak'Anon)
...
sub EVENT_TIMER
	...
	if ($timer == 1)
		{
		if ($skelup == "0")
			{    # If there are no tiny skeletons currently spawned
			quest::setglobal("skelatk", "$name", "3", "F")
				;    # Set global $skelatk with name of player to attack
			quest::spawn(13120, 106, 0, -1839.87, 829.51, -9.38)
				;    # Spawn tiny skeletons
			...
			quest::signalwith(13120, 1)
				;  # Signal tiny skeletons - tell them to attack player $skelatk
			}
		else
			{      # If any tiny skeletons are still spawned
