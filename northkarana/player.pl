...
sub EVENT_TIMER
	{
	if ($timer eq "nkspires")
		{
		if (defined $qglobals{nexus}
			&& defined $qglobals{spire_nk}
			&& $qglobals{spire_nk} == 1
		...
		elsif (defined $qglobals{nexus}
			   && defined $qglobals{spire_nk}
			   && $qglobals{spire_nk} == 1
			   && !defined $qglobals{message_nk}
			   && !plugin::check_hasitem($client, 19720))
			{
			$client->Message(15,"You don't have the correct component to travel to Luclin.");
			quest::setglobal(message_nk, 1, 1, "M20")
				;    #prevent component mssage from being spammed.
			$qglobals{nexus}      = undef;
			$qglobals{spire_nk}   = undef;
			$qglobals{message_nk} = undef;
			}
		elsif (defined $qglobals{nexus_nk}
			   && defined $qglobals{spire_nk}
			   && $qglobals{spire_nk} == 1
			   && defined $qglobals{message_nk}
			   && !plugin::check_hasitem($client, 19720))
...
sub EVENT_LOOT
	...
	if (($looted_id == 14344)
		...
		&& plugin::check_hasitem($client, 22892))
		{    #All 4 Paladin Heads
