...
sub EVENT_ITEM 
	...
	if ($faction > 5)
		{
		quest::say("Your reputation preceeds you. I will take no assistance.");
		}
	elsif(plugin::check_handin(\%itemcount, 18302 => 1))
		{ # Book of Scale
		...
		# Faction a guess from book of Turmoil
		quest::faction(402, 10);
		quest::faction(403, -10);
		quest::exp(72900); # 10% of level 10 xp
		quest::ding();
		}
	elsif(plugin::check_handin(\%itemcount, 18202 => 1))
		{ 
		# Book of Turmoil
		# Faction verified via zam
	...
	elsif (plugin::check_handin(\%itemcount, 55274 => 1)) 
