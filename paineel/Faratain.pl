...
sub EVENT_ITEM 
	...
	if (quest::istaskactivityactive(41, 7))
		{
		# Turning in container sets the can't do this again flag.
		if (plugin::check_handin(\%itemcount, 17124 => 1))
			{
			quest::updatetaskactivity(41,7);
			}
		}

	if (quest::istaskactivityactive(41, 14))
		{
		if (plugin::check_handin(\%itemcount, 20346 => 1))
			...
			if(!plugin::has_newbie_stuff($client))
				...
				if ($class eq "Shadowknight")
					{
					quest::enabletitle(12); 	# Shadowknight
					quest::shout2("All Hail Fell Blade $name!"); 
					}
				quest::updatetaskactivity(41,14);
				}
			else
				{
				quest::summonitem(20346);
				}
			} 
		}
	}
