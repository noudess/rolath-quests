function event_say(e)
	if (e.other.GetModCharacterFactionLevel(e.other, 265) < 0) then
		eq.reject_say(e);
	...
	elseif(e.message:findi("knight")) then
		...
		e.other:SummonItem(1792); -- Heavy Locked Chest
	end
end
...
function event_trade(e)
	...
	if (e.other.GetModCharacterFactionLevel(e.other, 265) < 0) then
		eq.reject_say(e);
	elseif(item_lib.check_turn_in(e.trade, {item1 = 1773})) then--Large Empty Crate
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 1776, item2 = 1777, item3 = 18174, item4 = 1778})) then--crate of ore,couriers head,suppliers head,sealed letter
		...
		e.other:Faction(265,20);  -- +Heretics
		e.other:Faction(242,-20);  -- -Deepwater Knights
		e.other:Faction(254,-20); -- -Gate Callers
		e.other:Faction(231,-20);  -- -Craftkeepers
		e.other:Faction(233,-20);  -- -Crimson Hands
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 1892})) then-- Empty Jar
		...
		e.other:Faction(265,40);  -- +Heretics
		e.other:Faction(242,-40);  -- -Deepwater Knights
		e.other:Faction(254,-40); -- -Gate Callers
		e.other:Faction(231,-40);  -- -Craftkeepers
		e.other:Faction(233,-40);  -- -Crimson Hands
