...
{	
	if ($signal == 1 && defined($qglobals{event_start}) && !defined($qglobals{wind_done})) 
	{
     	...
     	elsif ($signal == 2 && defined($qglobals{event_start}) && !defined($qglobals{wind_done})) 
     	{
     		...
     		if ($counter == 3) 
     		{
     	   		quest::spawn2(215449,0,0,-531.2,-552.3,105.1,510);
     	   		quest::spawn2(215449,0,0,-624.9,-651.1,109.1,338);
     	   		quest::spawn2(215448,0,0,-566.2,-836.3,105.1,26.4);
     	   		quest::spawn2(215449,0,0,-287.6,-882.7,107.0,499.8);
     	   		quest::spawn2(215449,0,0,-158.1,-560.4,105.1,433.8);
           		$counter=0;
     		}
     	}
