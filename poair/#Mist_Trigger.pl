...
sub EVENT_SIGNAL 
{
	if ($signal == 1 && defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) 
	{
     		...
     		if ($counter == 4) 
     		{
			quest::spawn2(215070,0,0,323.7,-721.3,439.3,165.8);
        	}
     	}
     	elsif ($signal == 2 && defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) 
     	{
        	quest::spawn2(215071,0,0,-1571.6,-570.0,343.6,381.8);
		quest::depop(215023);
     	}
	...
	elsif ($signal == 4 && !defined($qglobals{mist_event_start}) && !defined($qglobals{mist_done})) 
	{
		...
		{
			quest::spawn2(215069,0,0,304.2,-792.2,441.6,30.4); 
			quest::spawn2(215069,0,0,348.4,-632.2,442.3,280.2);
			quest::spawn2(215069,0,0,391.7,-736.2,438.1,406.8);
			quest::spawn2(215069,0,0,260,-699.8,444.5,151);
