...
sub EVENT_SIGNAL 
{
	if ($signal == 1 && defined($qglobals{smoke_event_start}) && !defined($qglobals{smoke_done})) 
	{
		$counter+=1;
     		...
     		if ($counter == 4) 
     		{
        		quest::spawn2(215454,0,0,-445.6,-1291.9,317.6,140.2);
        	}
     	}
     	elsif ($signal == 2 && defined($qglobals{smoke_event_start}) && !defined($qglobals{smoke_done})) 
     	{
     		quest::spawn2(215435,0,0,1395.9,-681.3,15.1,510.2);
        	quest::depop(215055);
     	}
	...
	elsif ($signal == 4 && !defined($qglobals{smoke_event_start}) && !defined($qglobals{smoke_done})) 
	{
		...
		{
			quest::spawn2(215066,0,0,-403.1,-1375.5,320.8,469.8); 
			quest::spawn2(215066,0,0,-501.9,-1205.9,318.6,198);
			quest::spawn2(215066,0,0,-341.1,-1245.2,317.6,365.6);
			quest::spawn2(215066,0,0,-523.3,-1288.2,321.7,126);
