...
sub EVENT_SAY 
	...
	if($text=~/retrieve key seven/i)
		...
		if ($faction >= 5)
			{
			quest::say("The Steel Warriors have no need to dislike you, but you have yet to truly prove your worth to this guild.");
			}
		else
			{
...
sub EVENT_ITEM {
   if(plugin::check_handin(\%itemcount, 20031 => 1)){
      ...
      quest::exp(100);
      quest::ding();
      quest::summonitem(quest::ChooseRandom(5033, 5025));                       
   ...
   if(plugin::check_handin(\%itemcount, 13398 => 1)){ #npc likes you, Arena Lion Skin
	  ...
	  # Verified
      quest::faction(311,25);     # Steel Warriors better - biggish faction boost after all
      quest::faction(262,5);     # Guards of Qeynos better
      quest::faction(230,-3);     # Corrupt Qeynos Guards worse
      quest::faction(330,-3);    # Freeport Militia worse
      quest::faction(281,5);     # Knights of Truth better
      quest::ding
