...
if (plugin::check_handin(\%itemcount, 13064 => 2) && $gold == 3) { # a peice of rat fur
	...
	quest::ding();
	# Faction && XP verified Live
	quest::exp(50);
	quest::faction(121,2); # Qeynos Citizen
	}
...
if (plugin::check_handin(\%itemcount, 13054 => 3) && $gold == 4) { # giant rat pelt
	...
	quest::exp(75);
	quest::ding();
	# Faction verified ZAM
	quest::faction(121,5); # Qeynos Citizen
	}
	
if (plugin::check_handin(\%itemcount, 13065 => 1, 13071 => 1, 13033 => 1)) { # rat foot, rat whiskers and a honey mead 
	quest::say("Hey. look at this. some of my best work, if I do say so myself. Enjoy, and tell your friends");  
	quest::summonitem(1053); # rat's foot necklace
	quest::ding();
	# Faction && XP verified on Live
	quest::exp(75);
	quest::faction(121,1); # Qeynos Citizen
	}
