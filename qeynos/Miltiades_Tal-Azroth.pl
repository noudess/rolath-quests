...
sub EVENT_ITEM 
	...
	if (quest::istaskactivityactive($taskid, 14))
		{
		if (plugin::check_handin(\%itemcount, 27336 => 1))
			{ # robe is last step
			if(!plugin::has_newbie_stuff($client))
				...
				quest::shout2("All Hail $name of the Rainkeepers!"); 
