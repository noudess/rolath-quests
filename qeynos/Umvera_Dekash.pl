...
sub EVENT_ITEM
	...
	if (plugin::check_handin(\%itemcount, 18504 => 1, 18505 => 1, 18506 => 1, 18507 => 1))
		{    #Tanned Split Paw Skin, volumes 1-4
		...
		$client->AddLevelBasedExp(.293, 18);
		quest::ding();
		}

	elsif (plugin::check_handin(\%itemcount, 18510 => 1))
		{                            #Pawbook
