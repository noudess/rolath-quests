sub EVENT_SAY {
  ...
  if($text=~/donate to the temple of thunder/i) {
    quest::say("I will be glad to help you out.  The Knights of Thunder are a respected order.");
...
sub EVENT_ITEM 
	...
	if(plugin::check_handin(\%itemcount, 13383 => 1)) 
		{
		quest::say("Thank you my friend!  Every Koalindl must be accounted for, even the dead.  Rodcet Nife shall be pleased and I shall reward you.  Nothing much, just a small token of gratitude.");
		quest::summonitem(quest::ChooseRandom(13297,13296));
		quest::givecash(0,0,3,1);
		# Faction and XP from Zam and P1999 wiki respectively
		quest::exp(17411);
		quest::faction(341, 100); 	# PoL
		quest::faction(280, 30);	# KoT
		quest::faction(262, 50);	# GoQ
