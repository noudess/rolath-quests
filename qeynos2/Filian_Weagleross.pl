...
sub EVENT_ITEM
{
	if (plugin::check_handin(\%itemcount, 27421 => 1, 27420 => 1, 27409 => 2)) 
		{
	...
	if (quest::istaskactivityactive(45, 7))
	{
		if (plugin::check_handin(\%itemcount, 17267 => 1))
		{
			quest::updatetaskactivity(45,7);
		}
	}

	if (quest::istaskactivityactive(45, 14))
		{
		if (plugin::check_handin(\%itemcount, 27357 => 1))
			{ # robe is last step
			if(!plugin::has_newbie_stuff($client))
				{
				quest::incstat(0,3);	# STA
				...
				if ($class eq "Cleric")
					...
					quest::shout2("All Hail Prime Healer $name!."); 
					}
				quest::updatetaskactivity(45,14);
				}
			else
				{
				quest::summonitem(27357);
				}
			} 
		}
