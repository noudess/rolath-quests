...
sub EVENT_ITEM 
	...
	if (quest::istaskactivityactive(48, 7))
		{
		# Turning in container sets the can't do this again flag.
		if (plugin::check_handin(\%itemcount, 17270 => 1))
			{
			quest::updatetaskactivity(48,7);
			}
		}

	if (quest::istaskactivityactive(48, 14))
		{
		if (plugin::check_handin(\%itemcount, 27378 => 1))
			{ # robe is last step
			if(!plugin::has_newbie_stuff($client))
				{
				quest::incstat(1,13); #STA
				quest::incstat(3,9);  #DEX
				if ($class eq "Monk")
					{
					quest::enabletitle(17); 	# Monk
					quest::shout2("All Hail $name of the Silent Fist!."); 
					}
				quest::updatetaskactivity(48,14);
				}
			else
				{
