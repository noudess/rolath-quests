...
sub EVENT_ITEM 
	...
	if (quest::istaskactivityactive(57, 7))
		...
		if (plugin::check_handin(\%itemcount, 17125 => 1))
			{
			quest::updatetaskactivity(57,7);
			}
		}

	if (quest::istaskactivityactive(57, 14))
		{
		if (plugin::check_handin(\%itemcount, 20257 => 1))
			{ # robe is last step
			if(!plugin::has_newbie_stuff($client))
				...
				quest::enabletitle(27);
				quest::shout2("All Hail Pine Druid $name!."); 
	
				quest::updatetaskactivity(57,14);
				}
			else
				{
