...
sub EVENT_SAY 
	...
	if($text=~/hail/i) 
		{
		if(quest::istaskactivityactive(12,2)) 
			{ #Task: Extraordinary Rodents
	...
	if($text=~/task/i) 
		{
		if($ulevel > 4 && $ulevel < 9) 
			{ #Maximum level for task is 8. Minimum level is currently unknown.
			if(quest::istaskactive(12)) 
				{ #Task: Extraordinary Rodents
...
sub EVENT_ITEM 
	{
	if($faction <= 4 && plugin::check_handin(\%itemcount, 13182 => 1)) 
		{ #Kevlin's Debt
