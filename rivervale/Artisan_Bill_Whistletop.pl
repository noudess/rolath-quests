...
sub EVENT_SAY {
	if($race eq "Halfling")
	{
		...
		if($text=~/beholder's maze/i)
		{
			if($ulevel >= 30)
			{
				quest::taskselector(178);
			}else
			{
				quest::say("I'm sorry, I don't think you are ready for that task yet. Please come back once you've gotten a bit more experience.");
			} 
		} # if($text=~/beholder's maze/i)		
		if($text=~/the hole/i)
		{
...
sub EVENT_ITEM {
	if($race eq "Halfling")
	{
		...
		{ 
			quest::say("Oh, hey there, $name. I'm still working on my history book. In fact, I just wrote down the part	where we first met. Here let me read it for you!");
			quest::emote("clears his throat.");
			quest::say("Ahem, so I was writing my history book when Dryssa comes up to me and says 'Hail.' And so I reply...");
