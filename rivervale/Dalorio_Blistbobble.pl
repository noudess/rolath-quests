...
sub EVENT_ITEM 
	...
	if (plugin::check_handin(\%itemcount, 22940 => 1, 22941 => 1, 22942 => 1, 22943 => 1)) 
		{
		quest::say("Your good deeds shall be echoed in the Rivervale Hall of Heroes for many years to come. With these documents I will be able to have my army prepared for whatever they decide to come at us with. I thank you, young $name. Please take this shard as a symbol of your loyalty to the Vale!");
    ...
    if (quest::istaskactivityactive($taskid, 14))
        ...
        if (plugin::check_handin(\%itemcount, $robeid => 1))
            ...
            if (!plugin::has_newbie_stuff($client))
                ...
                quest::enabletitle(41);
                quest::shout2("All Hail Vale Guardian $name!");
