...
if($text=~/I am new to the squad/i && $faction == 1){
quest::say("You are too modest $name!  You may just be [one with the wall]");
}
if($text=~/one with the wall/i && $faction < 6 && $faction > 1){
quest::say("You are in good standing with the Guardians of the Vale. Continue with your good work and then we may speak.");
}
if($text=~/one with the wall/i && $faction == 1){
