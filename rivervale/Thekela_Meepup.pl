...
sub EVENT_SAY 
	...
	if($text=~/what research annals/i)
		{
		quest::say("The research annals that I need you to retrieve are very important ones to me. King Tearis Thex of Felwithe presented them to me for study. He entrusted me with them because he felt I could benefit from the knowledge that they contained. Will you [seek out the annals]?");
		}

	if($text=~/i will seek out the annals/i)
		{
...
sub EVENT_ITEM 
	...
	if (plugin::check_handin(\%itemcount, 22668 => 1, 22696 => 3)) 
		{
		quest::say("I knew that I could count on you to follow through in your training. You have surely proven yourself in the time that you have trained with me. Please take this magical shard, as Bristlebane has blessed you with the power to give life to those that have fallen. Thank you, $name. You have made me very proud!");
    ...
    if (quest::istaskactivityactive($taskid, 14))
        ...
        if (plugin::check_handin(\%itemcount, $robeid => 1))
            ...
            if (!plugin::has_newbie_stuff($client))
                ...
                quest::enabletitle(38);
                quest::shout2("All Hail Templar $name of the Stoutdeacons!");
