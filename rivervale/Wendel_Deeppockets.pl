...
sub EVENT_SAY 
	...
	if($text=~/hail/i) 
		{
		quest::say("Its nice to meet you $name. Allow me to introduce myself. I am Wendel Deeppockets Master Assassin of Rivervale. I am personally in charge of all [training] we have the new recruits go through before they are sent out on specific missions.");
		}
	if($text=~/training/i) 
		{
	...
	if($text=~/job/i) 
		{
		quest::say("Orcs is the job $name. Plain and simple I cannot stand these homewreckers. They have moved into our land and continue to storm our front gate. I seek out bone fragments and fractured jawbones from these nasty Orcs. Once we have 1 of each we will be able to create a magical shard for you to enhance your weapon.");
		}
	}
    ...
    if (quest::istaskactivityactive($taskid, 14))
        ...
        if (plugin::check_handin(\%itemcount, $robeid => 1))
            ...
            if (!plugin::has_newbie_stuff($client))
                ...
                quest::enabletitle(37);
                quest::shout2("All Hail Scout $name of the Deepockets!");
